...
export class GitService {
  async getStatus(repoPath: string): Promise<GitStatus> {
    ...
    for (const file of status.files) {
      if (file.index !== " " && file.index !== "" && file.index !== "?") {
        ...
        staged.push({
          path: file.path,
          status: file.index as FileStatus,
          ...stats,
        });
      }
      if (file.working_dir !== " " && file.working_dir !== "") {
        ...
        unstaged.push({
          path: file.path,
          status: file.working_dir as FileStatus,
          ...stats,
        });
      }
    }
  ...
  async getUnstagedDiff(repoPath: string): Promise<string> {
    const git = simpleGit(repoPath);
    const [diff, status] = await Promise.all([git.diff(), git.status()]);

    const untrackedPaths = status.files
      .filter((f) => f.working_dir === "?" && f.index === "?")
      .map((f) => f.path);

    if (untrackedPaths.length === 0) return diff;
    ...
    if (syntheticDiffs.length === 0) return diff;
    const separator = diff && !diff.endsWith("\n") ? "\n" : "";
    return diff + separator + syntheticDiffs.join("\n");
  }
